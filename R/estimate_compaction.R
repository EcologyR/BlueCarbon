...
estimate_compaction <- function(df = NULL,
  ...
  if (any(df$compression < 0 | df$compression > 100)) {
    stop("Estimated compression below 0 or above 100: please check cores ",
         df$core[df$compression < 0 | df$compression > 100])
  }
