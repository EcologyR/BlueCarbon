...
The first dataframe ([core_comp](reference/core_comp.html)) has field measurement data that we will use to estimate soil compaction at core collection.

The second dataframe ([bluecarbon_data](reference/bluecarbon_data.html)) has core field information and laboratory data that we will use to correct core compaction, estimate organic carbon content in each sample from organic matter content, and to estimate the organic carbon stocks and sequestration rates.
...
We use function [decompact()](reference/decompact.html) to estimate the decompacted minimum and maximum depth of each sample before stock estimation. Furthermore, as compaction reduces the volume of the samples, dry bulk density has to be corrected. This can be done by estimating the corrected volume by: multiplying the corrected thickness of the sample (corrected maximum depth - corrected minimum depth) by the area of the core; or, if compacted dry bulk density is provided, function [decompact()](reference/decompact.html) will correct it. To provide compacted dry bulk density the name of the column has to be specified in the function parameter "dbd" as, by default, this parameter is NULL and the function assumes that it was not provided.

[decompact()](reference/decompact.html) has 6 parameters:

-   "df": data.frame with core properties

-   "core": (character) Name of the column with the id of the core to which the sample belongs

-   "compaction": (character) Name of the column with core compaction IN PERCENTAGE, as calculated with [estimate_compaction()](reference/estimate_compaction.html).
...
The function [estimate_oc()](reference/estimate_oc.html) has 7 parameters:
